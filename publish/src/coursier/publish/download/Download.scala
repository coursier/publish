...
object Download {
  ...
  object Error {
    final class HttpError(
      ...
      response: String
    ) extends Error(transient = code / 100 == 5, s"$url: HTTP $code\n$response")
    ...
    final class DownloadError(url: String, exception: Throwable)
        extends Error(transient = false, s"Download error for $url", exception)
